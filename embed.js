...
function poolsList(){
  ...
  // keep the objection pools side by side: Avoiding Objections right after Objection Control
  var ai = cats.indexOf('Avoiding Objections'), oi = cats.indexOf('Objections');
  if(ai !== -1 && oi !== -1){ cats.splice(ai, 1); cats.splice(cats.indexOf('Objections') + 1, 0, 'Avoiding Objections'); }
  // Underwriting Scenarios sits right after Product Knowledge
  var ui = cats.indexOf('Underwriting'), pi = cats.indexOf('Product knowledge');
  if(ui !== -1 && pi !== -1){ cats.splice(ui, 1); cats.splice(cats.indexOf('Product knowledge') + 1, 0, 'Underwriting'); }
  var pools = [];
