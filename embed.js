...
function renderPlay(){
  ...
  if(!S.me){
    ...
    var form = S.loginMode === 'code'
      ...
      : '<form id="join" autocomplete="off"><p class="hint">Enter your team\'s code and your name. Your host has the code for the room you are in.</p>' +
        ...
        '<button class="btn" type="submit">Join</button><p class="formerr" id="jerr" role="alert">' + esc(S.loginErr || '') + '</p></form>';
...
root.addEventListener('click', function(e){
  ...
  else if(act === 'signout'){ api('POST', '/api/logout').catch(function(){}).then(function(){ setToken(null); S.me = null; S.admin = false; S.openId = null; S.cur = null; S.host.agents = null; S.host.teams = null; if(S.view === 'host'){ S.view = 'board'; } $('#v-play').dataset.mode = ''; $('#v-host').dataset.mode = ''; syncViews(); }); }
  else if(act === 'hostout'){ api('POST', '/api/logout').catch(function(){}).then(function(){ setToken(null); S.admin = false; S.me = null; S.host.agents = null; S.host.teams = null; S.view = 'board'; $('#v-host').dataset.mode = ''; $('#v-play').dataset.mode = ''; syncViews(); }); }
...
root.addEventListener('submit', function(e){
  ...
  if(e.target.id === 'join'){
    e.preventDefault();
    var tc = $('#tc').value, jn = $('#jn').value, jerr = $('#jerr');
    if(!tc.trim()){ jerr.textContent = 'Enter your team code.'; return; }
    if(!jn.trim()){ jerr.textContent = 'Enter your name.'; return; }
    api('POST', '/api/join', {teamCode:tc, name:jn}).then(function(r){ setToken(r.token); S.me = r.agent; S.loginErr = ''; S.openId = null; $('#v-play').dataset.mode = ''; renderHeader(); renderPlay(); })
